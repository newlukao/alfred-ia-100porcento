...
serve(async (req) => {
  ...
  try {
    ...
    // Em produção, use Redis ou banco para rate limiting
    // Por enquanto, limitação simples por requisição
    ...
    const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${openaiApiKey}`, // ✅ Seguro - só no servidor!
...
1. No Supabase Dashboard → Settings → Environment Variables:
...
3. Teste:
   curl -X POST 'https://mxxkptkdekcqfncfbryv.supabase.co/functions/v1/chat' \
